...
export function interpretBlock(block, chip){
    switch(block.type){
        case -1: return;    //Space
        case 0: //Operator
            var left = block.left?interpretBlock(block.left,chip):false;
            var right=interpretBlock(block.right,chip);
        ...
        case 1: //SingleSided Operator
            switch(block.value){
                case "++": case "--":
                    ...
                    var initial=interpretBlock(actingOn,chip);
                ...
                case "!":
                    var left = interpretBlock(block.left,chip);
        ...
        case 4: //Keyword
            switch(block.value){
                ...
                case "or":
                    ...
                    return (left.value!=0 || right.value!=0)?{type:3,subtype:1,value:1}:{type:3,subtype:1,value:0};
                case "if":
                    var cond=interpretBlock(block.condition,chip);
                    if(cond.type==3 && cond.value!=0){
                        block.ifTrue.forEach(cmd=>{
                          interpretBlock(cmd,chip);
                        })
